...
const sortOptions: { id: SortOption; label: string; description: string }[] = [
  ...
  { id: 'queueTime', label: 'Queue Time', description: 'Longest waiting cases first' },
  { id: 'lastActivity', label: 'Last Activity', description: 'Recently updated cases first' },
  { id: 'alphabetical', label: 'Alphabetical', description: 'Sort by client name A-Z' },
];
      ...
      <div className="max-w-4xl mx-auto">
          ...
          <div className="relative" ref={filterDropdownRef}>
            <button
              ...
              className={cn(
                ...
                filterDropdownOpen && "bg-white/80 border-[#99C0F0]/60 shadow-md"
              )}
            >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 flex-1">
                        <div className={cn("w-3 h-3 rounded-full", option.color.dot)} />
                        <span className="text-sm text-[#0E315C]">{option.label}</span>
                      </div>
                      {isActive && (
                        <div className="w-2 h-2 bg-[#99C0F0] rounded-full" />
                      )}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-sm font-medium text-[#0E315C]">{option.label}</div>
                          <div className="text-xs text-[#0E315C]/60 mt-0.5">{option.description}</div>
                        </div>
                        {isActive && (
                          <div className="w-2 h-2 bg-[#C5BFEE] rounded-full" />
                        )}
        ...
        {(hasActiveFilters || hasActiveSearch) && (
          <div className="mt-3 flex items-center justify-center gap-2 text-xs text-[#0E315C]/60">
            ...
            {activeFilters.map(filterId => {
              ...
              return option ? (
                <span key={filterId} className="px-2 py-1 bg-[#99C0F0]/20 rounded-md flex items-center gap-1">
        ...
        <div className="text-center mt-4">
          <p className="text-[#0E315C]/60 text-xs leading-relaxed max-w-md mx-auto font-light">
            Search, filter, and sort cases to prioritize urgent reviews and document requests
          </p>
        </div>
      </div>
    </div>
